...
void pwm_setDutyCycle(uint8_t duty) {
    if (duty <= 100) {
        TIM14->CCR1 =
            ((uint32_t)duty * TIM14->ARR) / 100; // Use linear transform to produce CCR1 value
...
void PI_update(void) {
    ...
    // My motor says 540-ish pulses per revolution, but reports more like 625-ish pulses.
    // It also only has a max RPM of 48 at 6 Volts.
    // motor_speed is count of ENCA and ENCB posedge's and negedge's
    error = (target_rpm * 625 * 4 / 60) - (motor_speed * 20);
     ...
     *
     * Hint: If you chose 3200 for the integral clamp you should divide by 32 (right shift by 5
     * bits), this will give you an output of 100 at maximum integral "windup".
     *
